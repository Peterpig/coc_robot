...
class 自动化机器人:
    ...
    def _任务流程(self):
        ...
        try:
            ...
            # 从任务参数表读取运行模式（优先），回退到机器人设置（兼容旧数据）
            运行模式参数 = self.数据库.获取任务参数(self.机器人标志, "运行模式")
            if 运行模式参数:
                是否刷天鹰火炮 = 运行模式参数.get("是否刷天鹰火炮", False)
                是否刷主世界 = 运行模式参数.get("是否刷主世界", True)
                是否刷夜世界 = 运行模式参数.get("是否刷夜世界", False)
            else:
                是否刷天鹰火炮 = self.设置.是否刷天鹰火炮
                是否刷主世界 = self.设置.是否刷主世界
                是否刷夜世界 = self.设置.是否刷夜世界
