...
from 任务流程.任务元数据注册 import 升级城墙元数据
...
class 城墙升级任务(基础任务):
    ...
    def 检查功能开启(self, 上下文) -> bool:
        """检查是否开启刷墙功能"""
        是否开启 = self.参数.开启刷墙 if hasattr(self.参数, '开启刷墙') else 上下文.数据库.获取机器人设置(上下文.机器人标志).开启刷墙
        if not 是否开启:
            上下文.置脚本状态("刷墙功能已关闭")
            return False
        else:
    ...
    def 已够资源升级(self)-> bool:
        ...
        刷墙起始金币 = self.参数.刷墙起始金币 if hasattr(self.参数, '刷墙起始金币') else 上下文.数据库.获取机器人设置(上下文.机器人标志).刷墙起始金币
        刷墙起始圣水 = self.参数.刷墙起始圣水 if hasattr(self.参数, '刷墙起始圣水') else 上下文.数据库.获取机器人设置(上下文.机器人标志).刷墙起始圣水
