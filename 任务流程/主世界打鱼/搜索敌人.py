...
from 任务流程.任务元数据注册 import 主世界打鱼元数据
...
class 搜索目标敌人任务(基础任务):
    ...
    def 执行(self) -> bool:
        try:
            ...
            搜索次数 = 1
            # 从元数据参数获取，回退到机器人设置
            最小资源 = self.参数.欲进攻的最小资源 if hasattr(self.参数, '欲进攻的最小资源') else 上下文.数据库.获取机器人设置(上下文.机器人标志).欲进攻的最小资源
            原始目标 = 最小资源
    ...
    def 执行单次搜索(self, 上下文: 任务上下文, 搜索次数: int, 最小资源: int) -> bool:
        ...
        if 资源数据["总资源"] >= 最小资源:
            # 从元数据参数获取，回退到机器人设置
            靠近比例 = self.参数.欲进攻资源建筑靠近地图边缘最小比例 if hasattr(self.参数, '欲进攻资源建筑靠近地图边缘最小比例') else 上下文.数据库.获取机器人设置(上下文.机器人标志).欲进攻资源建筑靠近地图边缘最小比例
